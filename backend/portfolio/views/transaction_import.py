...
class BinanceTransactionHistoryImportView(APIView):
    """
    Upload a Binance Transaction History .xlsx statement; import fiat buy/sell trades.
    """
    ...
    def post(self, request):
        ...
        try:
            rows = parse_binance_transaction_history_xlsx(upload)
        except ValueError as e:
            return Response({"detail": str(e)}, status=status.HTTP_400_BAD_REQUEST)
        except Exception:
            logger.exception("Binance transaction history parse failed")
            return Response(
                {"detail": "Could not read the spreadsheet. Check that it is a valid Binance Transaction History export."},
                status=status.HTTP_400_BAD_REQUEST,
            )
